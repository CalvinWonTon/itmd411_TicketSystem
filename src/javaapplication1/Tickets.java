...
public class Tickets extends JFrame implements ActionListener {
	...
	public Tickets(Boolean isAdmin) {
		if(isAdmin != chkIfAdmin) {
			try {
				...
				add(sPane);
				setVisible(true);
				System.out.println("Tickets view successfully created");
				chkIfAdmin = true;
	...
	private void prepareGUI() {
		...
		if(chkIfAdmin == true) {
			bar.add(mnuAdmin);
		} // shows Admin menu only to admin
		bar.add(mnuTickets);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == mnuItemExit) {
			...
			// get ticket ID and update description
			String tickID = JOptionPane.showInputDialog(null, "Enter ticket ID to edit description");
			String updatedTicketDesc = JOptionPane.showInputDialog(null, "Enter an updated description");
			
			// update ticket information in database
			dao.updateRecords(tickID, updatedTicketDesc);
			
			// display results if successful
			int tID = Integer.parseInt(tickID);
			if(tID != 0) {
				System.out.println("Ticket ID: " + tID + " updated succesffuly!");
